        ...
        for i in range(num_groups - 1):
            for j in range(i + 1, num_groups):
                ...
                for feat in features:
                    ...
                    _, l = stats.levene(df_i[feat].values, df_j[feat].values)
            
                    if l > 0.05:
                        _, p = stats.ttest_ind(df_i[feat].values, df_j[feat].values, equal_var=True)
                    else:
                        _, p = stats.ttest_ind(df_i[feat].values, df_j[feat].values, equal_var=False)
