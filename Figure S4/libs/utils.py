...
def get_group_figure_adult(
    ...
    for i, feat in enumerate(feature_order):
        ...
        # relative 
        df_criteria = dataset[dataset['Label'] == 'aD'].drop(['Name', 'Label'], axis=1)
        dataset_tmp = dataset.drop(['Name', 'Label'], axis=1)
        dataset_tmp /= np.mean(df_criteria, axis=0)
        dataset_rev = pd.concat([dataset[['Name', 'Label']], dataset_tmp], axis=1)
...
def mean_ci(data, confidence=0.95):
    n = len(data)
    mean = np.mean(data)
    se = stats.sem(data)  # 표준 오차 계산
    h = se * stats.t.ppf((1 + confidence) / 2., n-1)  # 신뢰 구간
